...
def index(request):
    if request.method == 'GET':
        ...
        parser.add_argument('--model', '-m', default='media/hasui_01_2.model', type=str)
        parser.add_argument('--out', '-o', default='media/output.png', type=str)
        ...
        start = time.time()
        original = Image.open(photo.image).convert('RGB')
        ...
        med.save("media/output.png")




        params = {'form' : PhotoForm(),"input":input_photo,"result":args.out}
        return render(request,'myapp/result.html',params)

def explain(request):
    return render(request,"myapp/explain.html")
